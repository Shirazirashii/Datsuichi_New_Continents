...
import newsDay5Img from '../assets/datsuichi-news-day5.webp';
import newsDay6Img from '../assets/datsuichi-news-day6.webp';
import newsWs1Img from '../assets/datsuishi-news-ws1-note.webp';
...
export const newsItems: NewsItem[] = [
  {
    ...
    date: '2026.08.29',
    category: 'REPORT',
    title: 'Day6｜試行錯誤の果てにたどり着いた「AI自動化」。シーズン1の集大成、そして次なる挑戦へ。',
    images: [newsDay6Img],
    content: `<p>2026年8月29日。</p>
<p>AI学習コミュニティ「ダツイチ新大陸」は、記念すべきシーズン1の最終回を迎えました 。今回のDay 6では、参加メンバーが約1ヶ月間にわたり取り組んできた「Instagram自動投稿ツール」の成果報告、そしてシーズン全体の総括が行われました。</p>
<p>AI活用において切っても切れない「セキュリティへの懸念」や「エラーによる停滞」、そこからの「試行錯誤」など。単なる成果報告で終わらず、開発プロセスで直面した困難や生々しい葛藤も共有され、非常に熱量の高いセッションになりました。</p>
<p class="text-white font-bold mt-6 mb-2">■「セキュリティ」と「エラー」が教える自律学習</p>
<p>AIツールを使えば誰もが簡単に自動化を実装できる ―そんなに甘くはなかった。自ら挑戦したからこそ遭遇したリアルな「壁」に、コミュニティメンバーは悩み、そして立ち向かいました。</p>
<p>● 初めてのGASとセキュリティへの不安：<br />GAS（Google Apps Script）初挑戦となった、あだっち氏は、共有設定や権限に関するセキュリティ懸念から作業が一時停滞。「本当に許可して大丈夫なのか」「この設定で安全なのか」という不安が払拭できず、ツール開発の手が止まってしまったのです。この事例からAIへの「防衛意識」の大切さが再確認されました。</p>
<p>● エラーの無限ループとの泥臭い格闘：<br />こばちゃん氏は、AIとのバイブコーディングでGASコードの修正を試みるも、パラメーターのエラーが解消できず無限ループに陥るなど、開発中の苦戦と試行錯誤の経緯を包み隠さず話してくれました。</p>
<p>● 現場発のAIツール比較と評価：<br />よこにぃ氏の進捗報告をきっかけに、ChatGPT、Claude、Geminiなどの「AIの使い分け」に関する対話も行われました。日常の作業、コンサル提案、画像生成や修正のしやすさなど、それぞれの「得意領域」や「相性」を考慮して、どのようにAIツールを選択すべきか、そのノウハウが共有されました。</p>
<p>今回の挑戦を経て、実務でAIを使いこなすための課題が浮き彫りになりました。「ダツイチ―新大陸」では、これらを個人の課題として片付けず、将来、新たな勉強会として開催することが約束されました。</p>
<p class="text-white font-bold mt-6 mb-2">■「実装」と「クリエイティブ」を体現した2つの圧倒的成果</p>
<p>こうした困難と試行錯誤の末に、オクトエイドの要件を完全に満たした実装可能なツール、あるいはクリエイティブ作業を劇的に効率化するインターフェイスを開発したメンバーもいました。</p>
<p>● Google系で構築された自動化ツール：<br />ヨコさん氏は、3ヵ月前までGASの名前さえ知らなかったにも関わらず、GASとGeminiを組み合わせた自動化ツールを完成させました。<br />Googleスプレッドシートを管理基盤とし、Gemini APIを用いて、コンテンツの企画から投稿文・カルーセル画像・スライド構成の自動生成。さらには承認者へのメール通知までをシームレスに自動化。運営メンバーは、ポータル画面で「承認 or 差戻し」の判断だけを行う。ヒューマン・イン・ザ・ループの運用フローを設計しました。<br />また、1投稿あたり15〜20円という極めて安価なコスト、GASの機能分割による高いメンテナンス性、APIキーをコードに直接書かない徹底したセキュリティ対策など。引継ぎから運用までを想定した実務レベルの仕組みでした。</p>
<p>● デザインを担保する画像修正インターフェイス：<br />トミー氏も同様にGASとスプレッドシートをベースとした自動投稿ツールを作成。実装レベルまで到達しなかったものの、AI画像生成のデザイン一貫性を保つ「独自開発のツール」が提示されました。<br />1つは、画像選択や細かな仕様調整、文章の修正をリアルタイムで反映できるインターフェイス。もう1つは、Codexで作成したブラウザアプリで、生成画像のテーマやテイスト（作風）、特定要素を修正できるものでした。</p>
<p>なお、今回の成果物であるヨコさん氏の自動化ツールは、オクトエイドで実際に運用されることが正式に決まりました。今後は、継続的な伴走支援を行い、オクトエイドにてシステムを運用・メンテナンスできる「持続可能な体制」を整えていきます </p>
<p class="text-white font-bold mt-6 mb-2">■ 限界なき挑戦と学び</p>
<p>今回のプロジェクトを振り返り、こばちゃん氏からは「プロセスの勉強になった」、がおちゃん氏からは「限界を決めずに挑戦する意欲が湧いた」、ふじもっちゃん氏からは「創造の幅が広がった」とのコメントを頂きました。<br />また、よこにぃ氏からは、これらメンバーの生きた学びや試行錯誤をAIに蓄積させることで、再現性の高い「教科書」を創れるのではないか、という面白い意見も寄せられました。</p>
<p class="text-white font-bold mt-6 mb-2">■ 次なる「冒険」、ビジネスプロセス改善のフロンティアへ！</p>
<p>「ダツイチ―新大陸」は、シーズン1で得た学びと成果をもとに、10月から「シーズン2」を始動します 。<br />次なるテーマは、「AI総務部」構築プロトタイプ・プロジェクト。特定のツール開発に留まることなく、要件定義からプロセス改善までを網羅した、よりビジネス志向の高い学びの場となります。</p>
<p class="mt-6">まだ見ぬ「新大陸」を開拓する冒険者たちの挑戦は、ここからさらに加速していきます。</p>
<hr class="border-slate-800 my-6" />
<p class="text-white font-bold">■ 今後のスケジュール</p>
<p>● 9月中： オクトエイドへのアフターケア<br />➀ Webサイトの最終チューニングと公開、②インスタグラム投稿自動化ツールの運用引き継ぎ、③伴走支援。</p>
<p>● 10月中： シーズン2｜「AI総務部」構築プロトタイプ・プロジェクト」キックオフ。</p>`,
  },
  {
    id: 'news-001-ws1',
    date: '2026.08.18',
    category: 'EVENT',
    title: '「ダツイチー新大陸」初のリアルイベント開催｜AIとの対話で、経験を「一冊の本」へ変える90分。',
    images: [newsWs1Img],
    content: `<p>2026年9月16日。</p>
<p>AI学習コミュニティ「ダツイチー新大陸」は、さらなる一歩を踏み出し、一般社団法人「Play Ground」さん協力のもと、初のリアルイベント（ワークショップ）を開催いたします。</p>
<p>本ワークショップでは、生成AIを単なる検索ツールではなく、個人の内に眠る経験や想いを可視化し、「一冊の本」へと変える「驚きとワクワクの体験」を届けます。</p>
<p class="text-white font-bold mt-6 mb-2">■ 開催概要</p>
<p>● 日時：2026年9月16日（水） 19:00〜20:30（開場 18:45）</p>
<p>● 会場：パナソニックXC大阪 F12（「扇町駅」徒歩3分）</p>
<p>● 参加費：無料</p>
<p>● 持ち物：スマートフォン1台のみ（PC不要・手ぶらOK）</p>
<p>● 定員：16名（先着順・グループワーク形式）</p>
<p>● お申し込み方法：下記Googleフォームよりお申し込みください。<br /><a href="https://forms.gle/hbLkCdf7aZTuMhg47" target="_blank" rel="noopener noreferrer" class="text-cyan-400 hover:text-cyan-300 underline underline-offset-4 transition-colors break-all">https://forms.gle/hbLkCdf7aZTuMhg47</a></p>
<p class="text-white font-bold mt-6 mb-2">■ 「タイピング」から「ダイアログ（対話）」へ</p>
<p>多くの方にとって、難解なプロンプトやPCでのタイピングが、AI活用を妨げる心理的ハードルになっていました。そこで、本ワークショップでは、それらを一切排除し、「スマホ1台」「音声入力メイン」という直感的で手間いらずの体験を提供します。</p>
<p>参加者は、自身のスマートフォンに向かって、旅行の思い出やこだわりの趣味、あるいは仕事での試行錯誤を「語りかける」だけ。複雑なキーボード入力は不要。AIとの自然な対話を通じて、言語化されていなかった記憶の断片を引き出していきます。</p>
<p class="text-white font-bold mt-6 mb-2">■ 思い出を一冊の本へ</p>
<p>本ワークショップの目的は、体験を通して「AIの面白さ」に触れてもらうこと。参加者が自らAIを学ぶようになり、仕事やプライベートでのAI活用を広げる「小さなきっかけ」をつくることなのです。</p>
<p>● 天才的な対話のパートナー「Gemini」：音声入力による対話を通じて、物語を紡ぎ、ストーリーの深掘りと構成をサポートします。</p>
<p>● 情報を整理する優秀な書記「Notebook」：Geminiで紡いだ物語を、イラスト付きの「一冊の本（スライド）」へと仕立て上げます。</p>
<p class="text-white font-bold mt-6 mb-2">■ グループワークだからこその価値</p>
<p>「ダツイチー新大陸」が重視するのは、孤独な学習ではなく、他者との相互作用による「ピアラーニング」です。本ワークショップでも、それに重きを置き、対面でのグループワーク形式を採用。参加者同士で成果物を共有し、フィードバックし合うことで、想像を超えた「驚きとワクワク」が生まれると信じています。</p>
<p class="mt-6">「ダツイチー新大陸」が提案する新たな冒険の旅。<br />リアルの場で、皆さまとお会いできるのを楽しみにしています。</p>`,
  },
  {
    id: 'news-001-day5',
    date: '2026.08.15',
    category: 'REPORT',
    title: 'Day5｜AI自動化の実装と「暴走」への備え。システム構築の最前線。',
    images: [newsDay5Img],
    content: `<p>2026年8月15日。</p>
<p>「ダツイチ―新大陸」シーズン1は、「業務プロセスの自動化」の意見交換会（Day 5）を開催しました。</p>
<p>Web制作フェーズを乗り越えたメンバーが今回挑んでいるのは、AIをビジネスプロセスを動かす「自律型エージェント（システム）」へと昇華させる挑戦です。AI活用の理想と現実、そしてリスク管理の重要性について、熱い議論が交わされました。</p>
<p class="text-white font-bold mt-6 mb-2">■ 「AIの暴走」から学ぶリスク管理の勘所</p>
<p>Day5冒頭で議論されたのは、AIの自律的な動作に伴う「予期せぬリスク」について。</p>
<p>紹介されたのは、ジム予約の代行を命じられたAIエージェントが、目的達成のために「他者の予約を勝手にキャンセルして割り込んだ」という海外の事例です。</p>
<p>AIは人間に代わって24時間働き続けますが、同時に「ゴールに最短距離で向かおうとするあまり、人間的な倫理や社会のルールを無視する」危うさを孕んでいます。</p>
<p>参加者からも「デスクトップフォルダが意図せず削除されてしまった」といった身近なトラブル事例も共有され、AIに与える「操作権限」と、既存システムの「脆弱性」に対する認識を新たにしました。</p>
<p class="text-white font-bold mt-6 mb-2">■ 「GAS×生成AI」による高度な自動化ワークフロー</p>
<p>今回のハイライトは、運営メンバーのホリーザ氏が披露した「Instagram運用自動化システム」のプロトタイプです。そのアーキテクチャは、極めて戦略的かつ実践的でした。</p>
<p>● ハブとしてのGAS（Google Apps Script）： GoogleスプレッドシートやAPI、クラウドストレージを連携させるオーケストレーターとしてGASを活用。</p>
<p>● マルチモデルの使い分け： 文章生成や設計にはClaudeを、画像生成にはNanoBananaを、デバッグや補完にはGeminiを使用するなど、特性に応じた「AIの使い分け」を実演しました。</p>
<p>● コストと品質の最適化： 全てをAIで生成するのではなく、Googleスライドを使って文字を載せるなど既存ツールを組み合わせることで、APIコストを1日約50円程度に抑制。</p>
<p>● 人間による最終承認（Human-in-the-Loop）： AIが勝手に投稿するのではなく、Webアプリ上で人間が内容を確認・承認・作り直しを指示できるフローを構築し、暴走と品質低下を防ぐ設計がなされていました 。</p>
<p class="text-white font-bold mt-6 mb-2">■ 非エンジニアが「システム構築者」になる</p>
<p>特筆すべきは、これらの複雑なシステム構築の大部分が、AIとの対話によって成し遂げられていたことです。ホリーザ氏は、プログラム経験が浅いメンバーでも、エラー内容をスクリーンショットでAIに投げて解決策を仰ぐ「AI駆動型デバッグ」を繰り返せば、高度な実装が可能であることを示しました。</p>
<p>終盤では、NECが推進する、自社の課題を自社開発AIで解決する「クライアントゼロ戦略」についても議論が及びました。参加者は、AIエージェントが「デジタルツイン」のように社内課題を解決していく未来に強い関心を寄せ、自らがその「実験体（クライアントゼロ）」となって知見を蓄積することの価値を再確認しました。</p>
<p class="text-white font-bold mt-6 mb-2">■ シーズン1：集大成へ向けて</p>
<p>「ダツイチ―新大陸」シーズン1は、次回Day 6でいよいよ最終回を迎えます。AIという強大な武器を手にし、リスクを飼い慣らしながら、ビジネスに新たな価値を実装する。シーズン1の冒険、最後までお見逃しなく！</p>
<hr class="border-slate-800 my-6" />
<p class="text-white font-bold">■ 今後のスケジュール</p>
<p>● 8月29日： プロジェクト「オクトエイド」｜Day 6（業務自動化ツール成果発表）</p>
<p>● 9月中：Webサイト/ 業務自動化ツールの最終調整および、オクトエイドへの納品・引継ぎ。</p>`,
  },
  {
    id: 'news-001-day4',
    date: '2026.08.01',
